...
): SavVariable['missingValues'] | undefined {
...
function extractRowsFromModule(
  mod: ReadStatModule,
 ...
 */
export async function parseSavFile(buffer: ArrayBuffer, onProgress?: ProgressCallback): Promise<SavParseResult> {
 ...
 */
export async function parseSavMetadata(buffer: ArrayBuffer): Promise<SavMetadataResult> {
  const startTime = performance.now();
...
export async function parseSavSample(
  ...
  try {
    ...
    const rows =
      strategy === 'spread' && safeLimit < fullRows.length
        ? (Array.from(
            { length: safeLimit },
            (_, i) => fullRows[Math.floor((i * fullRows.length) / safeLimit)] ?? fullRows[fullRows.length - 1],
          ).filter(Boolean) as (number | string | null)[][])
  ...
  console.log(
    `📦 [ReadStat] Parsing SAV sample (${safeLimit} rows, ${strategy}): ${(buffer.byteLength / 1024 / 1024).toFixed(2)} MB`,
  );
 ...
 */
export type StreamingProgressCallback = (batch: RowBatch) => Promise<void | number> | void | number;
...
export async function parseSavWindow(
  ...
  try {
    ...
    const windowRowCount = mod._get_window_row_count?.() ?? mod._get_parsed_row_count?.() ?? metadata.rowCount;
  ...
  try {
    ...
    while (processedRows < totalRows) {
      ...
      try {
        ...
        const windowRows = mod._get_window_row_count?.() ?? mod._get_parsed_row_count?.() ?? mod._get_row_count();
        ...
        if (typeof suggestedBatchSize === 'number' && Number.isFinite(suggestedBatchSize) && suggestedBatchSize >= 1) {
...
export async function parseSavStreaming(
  ...
  try {
    ...
    const durationMs = performance.now() - startTime;
    console.log(
      `📦 [ReadStat] Streaming complete: ${rowCount} rows in ${batchCount} batches, ${durationMs.toFixed(0)}ms`,
    );
...
export async function parseSavStreamingSinglePassBridge(
  ...
  try {
    parseResult = await parseSavStreaming(buffer, initialBatchSize, async (batch) => {
      while (credits <= 0) {
        if (consumerError) {
          throw consumerError;
        }
