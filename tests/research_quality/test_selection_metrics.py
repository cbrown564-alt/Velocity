...
def test_selection_metrics_known_fixture(tmp_path):
    ...
    matches = [
        ...
        },
        {
            ...
            "match_state": "unsupported",
            "model_importance": "secondary",
            "topic_cluster": "x",
        },
        {
            "model_finding_id": "m4",
    ...
    mp.write_text(json.dumps(matches))
    subprocess.run(
        [
            sys.executable,
            str(SCRIPT),
...
def test_independently_supported_novel_claim_does_not_inflate_recall_or_coverage(
    tmp_path,
):
    ref = {"findings": [{"finding_id": "A", "tier": "mandatory", "topic_cluster": "a"}]}
    matches = [
        {
            "model_finding_id": "m1",
            "reference_finding_id": None,
    ...
    o = json.loads(op.read_text())
    assert o["selected_precision"] == 1
    assert o["mandatory_recall"] == 0
    assert o["redundancy_adjusted_coverage"] == 0
