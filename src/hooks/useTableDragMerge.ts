...
export const useTableDragMerge = ({
    ...
    const handleDragStart = useCallback((item: TableDragItem, event: React.MouseEvent) => {
    ...
    useEffect(() => {
        ...
        const handleMouseUp = () => {
            if (dragState.dropTarget && dragState.draggedItem && onMerge) {
                ...
                const sourceItems = sourceKeys.map(key => {
    ...
    }, [dragState.isDragging, dragState.draggedItem, dragState.dropTarget, containerRef, onMerge, selectedRows, selectedCols]);
