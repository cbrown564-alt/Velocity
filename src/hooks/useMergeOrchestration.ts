...
export const useMergeOrchestration = (fallbackVariableId?: string) => {
  ...
  const confirmMerge = useCallback(
    async (groupName: string) => {
      ...
      const sourceColEntry =
        priorState.tableConfig.colVar !== null &&
        (() => {
      ...
      try {
        const newVarId = await recodeVariable(mergeModal.variableId, `${label} (Grouped)`, {
          mode: 'categorical',
          mappings,
        });
        ...
        if (sourceEntry || sourceColEntry) {
          const newRowVars = postState.tableConfig.rowVars.map((id) => (id === sourceEntry ? newEntry : id));
