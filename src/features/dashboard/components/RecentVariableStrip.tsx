...
export const RecentVariableStrip: React.FC<RecentVariableStripProps> = ({ onVariableClick }) => {
  ...
  const stripIds = useMemo(() => {
    const available = new Set(variableSets.map((set) => set.id));
    return buildStripVariableIds(
      pinnedIds.filter((id) => available.has(id)),
      recentIds.filter((id) => available.has(id)),
    );
  }, [pinnedIds, recentIds, variableSets]);
  ...
  return (
    ...
    >
      <button
        ...
        className="inline-flex items-center gap-1 shrink-0 h-6 px-1.5 rounded-md text-[10.5px] uppercase tracking-wide text-[var(--text-secondary)] hover:text-[var(--text-secondary)] hover:bg-[var(--bg-rail)] transition-colors"
      ...
      {collapsed && (
        <span className="text-[11px] text-[var(--text-secondary)] truncate" data-testid="recent-variable-strip-summary">
