    ...
    >
      <div className="flex items-start gap-1.5">
        <div className="min-w-0 flex-1 space-y-0.5">
          ...
          {summary.unresolvedVariableLabels.length > 0 && (
            <p
              ...
              data-testid="session-import-unresolved"
            >
              {unresolvedLabel}
          ...
          {summary.affectedSlideNumbers.length > 0 && (
            <p className="text-[11px] leading-snug text-[var(--text-secondary)]" data-testid="session-import-affected">
              Affects slides {formatSlideList(summary.affectedSlideNumbers)}
            </p>
          )}
          {secondaryAdjustments.map((item) => (
            <p key={item.id} className="text-[11px] leading-snug text-[var(--text-secondary)]">
              {item.message}
            </p>
          ))}
        </div>
        <button
          ...
          className="shrink-0 rounded p-0.5 text-[var(--text-secondary)] hover:text-[var(--text-secondary)] hover:bg-[var(--bg-panel)] transition-colors"
        >
