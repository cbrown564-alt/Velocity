...
import { Home, Table, BarChart3, Upload, Download, FileDown, RotateCcw, Database, MoreHorizontal } from 'lucide-react';
...
const OverflowMenu: React.FC<{
  ...
  onExportSession: () => void;
  onReset: () => void;
}> = ({ dataset, onOpenSessionImport, onExportSession, onReset }) => {
  ...
  <header className="surface-panel relative z-[var(--z-sticky)] h-14 border-b border-[var(--border-color-muted)] flex items-center justify-between px-6 bg-[var(--bg-panel)] shrink-0 overflow-visible">
    ...
    <div className="flex items-center gap-2 shrink-0">
      <div className="flex items-center bg-[var(--bg-rail)] p-1 rounded-lg">
        <button
          type="button"
          onClick={() => activeSlideId && useVelocityStore.getState().setSlideVisualizationType(activeSlideId, 'table')}
        ...
        <button
          type="button"
          onClick={() => activeSlideId && useVelocityStore.getState().setSlideVisualizationType(activeSlideId, 'chart')}
      ...
      <button type="button" onClick={() => useVelocityStore.getState().openCommandPalette()} className={GHOST_BUTTON}>
        Insert<kbd className={KBD_HINT}>⌘K</kbd>
      </button>
      <OverflowMenu
        dataset={dataset}
