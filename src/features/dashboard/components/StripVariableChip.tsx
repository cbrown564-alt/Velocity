...
export const StripVariableChip: React.FC<StripVariableChipProps> = ({
  ...
  return (
    <div className="inline-flex shrink-0 items-center">
      ...
      <button
        type="button"
        aria-label={`${pinned ? 'Unpin' : 'Pin'} ${variableSet.name}`}
        aria-pressed={pinned}
        className="h-7 px-1 rounded-md text-[var(--text-secondary)] hover:bg-[var(--bg-rail)]"
        onClick={(event) => onContextMenu?.(variableSet, event)}
      >
