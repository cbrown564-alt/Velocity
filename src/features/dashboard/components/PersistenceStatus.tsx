...
export const PersistenceStatus: React.FC<PersistenceStatusProps> = ({
  ...
  const labelCoveragePct =
    typeof labeledVariableCount === 'number' && typeof totalVariableCount === 'number' && totalVariableCount > 0
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="text-xs font-medium text-[var(--text-primary)] truncate">{statusLabel}</div>
          <div className="text-[10px] text-[var(--text-secondary)] truncate">
            {hasPartialLoad
              ? 'Value labels were partially dropped'
              : usageMb
                ? `${usageMb.toFixed(1)} MB used`
                : 'Ready'}
          </div>
        </div>
      </button>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 rounded-lg bg-[var(--bg-app)] border border-[var(--border-color-muted)]">
                    ...
                    <div className="flex items-center gap-2">
                      {opfsAvailable ? (
                        ...
                        <>
                          <AlertCircle size={14} className="text-[var(--status-warning-text)]" />
                  ...
                  <div className="p-3 rounded-lg bg-[var(--bg-app)] border border-[var(--border-color-muted)]">
                    ...
                    <div className="text-sm font-medium">
                      ...
                      {usagePct !== null && (
                        <span className="text-[var(--text-secondary)] text-xs ml-1">({usagePct}%)</span>
                      )}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="p-4 rounded-lg bg-[var(--bg-app)] border border-[var(--border-color-muted)] space-y-3">
                    ...
                    <div className="grid grid-cols-[1fr,auto] gap-y-2.5 gap-x-4 text-sm">
                      ...
                      <div className="text-right font-medium text-[var(--text-primary)]">
                        {datasetRows !== null && datasetRows !== undefined ? datasetRows.toLocaleString() : '--'}
                      </div>

                      <div className="text-[var(--text-secondary)]">Columns</div>
                      <div className="text-right font-medium text-[var(--text-primary)]">
                        {datasetColumns !== null && datasetColumns !== undefined
                          ? datasetColumns.toLocaleString()
                          : '--'}
                      </div>

                      <div className="text-[var(--text-secondary)] flex items-center gap-1.5">Estimated Cells</div>
                      <div className="text-right font-medium text-[var(--text-primary)]">
                        {estimatedCells !== null && estimatedCells !== undefined
                      ...
                      <div className="text-right font-medium text-[var(--text-primary)]">
                        ...
                        {typeof totalValueLabelCount === 'number' && (
                          <span className="text-[var(--text-secondary)] ml-1.5 text-xs font-normal">
                            ({totalValueLabelCount.toLocaleString()} labels)
                          </span>
                        )}
                      </div>
                      ...
                      <div
                        className={`text-right font-medium flex items-center justify-end gap-1.5 ${
                          memoryRisk === 'critical'
                    ...
                    <div className="mt-3 pt-3 border-t border-[var(--border-color-muted)] flex items-start gap-2 text-[var(--status-warning-text)] bg-[var(--status-warning-surface)] p-2.5 rounded-md">
                      ...
                      <div className="text-xs leading-relaxed">
                        Browser limits are based on total process memory, not only OPFS disk quota. High memory risk may
                        cause the tab to crash.
                      </div>
                    </div>
                  </div>
                )}
