...
export const useResolvedVariables = (): ResolvedVariables => {
  ...
  return useMemo(() => {
    ...
    const rowVars = tableConfig.rowVars.map(resolveVarSetToVariable).filter((v): v is Variable => v !== null);

    const colVar = tableConfig.colVar ? resolveVarSetToVariable(tableConfig.colVar) : null;

    const firstRowVarSet =
      tableConfig.rowVars.length > 0 ? variableSets.find((vs) => vs.id === tableConfig.rowVars[0]) : undefined;
