    ...
    >
      <div className={styles.itemContent}>
        ...
        <span className={styles.itemLabel}>
          ...
          {derivedFromName && (
            <span className={styles.lineageBadge} title={`Derived from ${derivedFromName} via recode`}>
              <GitBranch size={10} />
            </span>
          )}
        </span>
      ...
      <div className={styles.itemMeta}>
        ...
        {missingPercent !== undefined && <MissingnessBadge missingPercent={missingPercent} threshold={1} />}
        {structureLabel && <span className={styles.itemCount}>{structureLabel}</span>}
...
export const VariableSetColumn: React.FC = () => {
  ...
  return (
    <div className={`${styles.column} ${styles.col3}`} onMouseLeave={() => setHoveredVariableSetId(null)}>
      <div className={styles.columnHeader}>
        <span className={styles.columnTitle}>Variable Sets</span>
        <span style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)' }}>{filteredSets.length}</span>
      </div>
