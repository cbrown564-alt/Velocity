...
export const InspectorHeader: React.FC<InspectorHeaderProps> = ({ variable, stats, isLoadingStats }) => {
  ...
  return (
    <div className={styles.header}>
        ...
        <div className={styles.editableTitleContainer} style={{ borderBottom: 'none' }}>
          ...
          >
            {isEditingName ? (
              ...
              <>
                <span
                  className={styles.variableIdCode}
                  style={{ fontSize: '12px', padding: '3px 8px', backgroundColor: 'var(--bg-surface)' }}
                >
                  {variable.name}
                </span>
              </>
          ...
          <div
            ...
            className={styles.editableTitleContainerHoverTarget}
          >
            {isEditingLabel ? (
              <input
                ref={labelInputRef}
      ...
      <div
        className={styles.headerSubtitleRow}
        style={{
      ...
      >
        <div className={styles.headerBadges}>
          ...
          {!isNumeric && hasValueLabels && (
            <span className={styles.categoryCountBadge}>{variable.valueLabels!.length} categories</span>
          )}
          {isNumeric && stats?.numeric?.mean !== undefined && (
            ...
            >
              Mean:{' '}
              <strong style={{ color: 'var(--text-primary)', marginLeft: '2px' }}>
                {stats.numeric.mean.toFixed(2)}
              </strong>
            </span>
          )}
        </div>
        ...
        {isLoadingStats ? (
          ...
          <div style={{ display: 'flex', gap: '16px', fontSize: '11px', color: 'var(--text-secondary)' }}>
            <span>
              Valid{' '}
              <strong style={{ color: 'var(--text-primary)', fontWeight: 600 }}>{validCount.toLocaleString()}</strong>
            </span>
            {missingCount > 0 && (
              <span>
                Missing{' '}
