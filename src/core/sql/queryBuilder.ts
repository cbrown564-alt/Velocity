...
export function buildGridQuery(options: GridQueryOptions): string {
  ...
  const itemsValues = columns.map(({ label }, index) => `(${index}, '${escapeString(label)}')`).join(', ');
  ...
  if (aggregate) {
    ...
    groupByClause = colVar ? `GROUP BY item_label, ${colGroup}` : `GROUP BY item_label, ${colGroup}`; // Total is constant, so just item_label is enough, but constant works too
  } else {
...
function buildMultipleQuery(options: MultipleQueryOptions): string {
  ...
  const colSelector = colVar ? `"${escapeIdentifier(colVar)}"` : `'Total'`;
  ...
  const unionParts = columns.map(({ name, label, countedValue }) => {
    const parts = [`SELECT '${escapeString(label)}' as rowKey_0, ${colSelector} as colKey, ${statsExpr}`, `FROM main`];
...
function buildColumnMultipleQuery(options: ColumnMultipleQueryOptions): string {
  ...
  const unionParts = columns.map(({ name, label, countedValue }) => {
    const parts = [`SELECT ${rowSelectors}, '${escapeString(label)}' as colKey, ${statsExpr}`, `FROM main`];
...
export function buildCrosstabQuery(options: CrosstabQueryOptions): string {
  ...
  const parts = [`SELECT ${rowSelectors}, ${colSelector}, ${statsExpr}`, `FROM main`];
