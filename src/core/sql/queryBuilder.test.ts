...
describe('queryBuilder', () => {
  ...
  describe('buildCrosstabQuery', () => {
    ...
    it('includes WHERE clause when filters are provided', () => {
      const filters: Filter[] = [{ id: 'f1', variableId: 'Age', operator: 'gt', value: 18 }];
    ...
    it('includes sumSqWeights for weighted grid aggregation', () => {
      ...
      expect(sql).toContain(
        'SUM(CASE WHEN _synthetic_value IS NOT NULL THEN "weight" * "weight" ELSE 0 END)::DOUBLE as sumSqWeights',
      );
    ...
    it('builds a weighted measure query for scale variables', () => {
      ...
      expect(sql).toContain(
        '(SUM(CASE WHEN "Age" IS NOT NULL THEN "Age" * "weight" ELSE 0 END) / NULLIF(SUM(CASE WHEN "Age" IS NOT NULL THEN "weight" ELSE 0 END), 0))::DOUBLE as mean',
      );
      expect(sql).toContain(
        'SQRT(ABS((SUM(CASE WHEN "Age" IS NOT NULL THEN "Age" * "Age" * "weight" ELSE 0 END) / NULLIF(SUM(CASE WHEN "Age" IS NOT NULL THEN "weight" ELSE 0 END), 0)) - POWER(SUM(CASE WHEN "Age" IS NOT NULL THEN "Age" * "weight" ELSE 0 END) / NULLIF(SUM(CASE WHEN "Age" IS NOT NULL THEN "weight" ELSE 0 END), 0), 2)))::DOUBLE as stdDev',
      );
  ...
  describe('buildOverlapQuery', () => {
    ...
    it('builds weighted overlap SQL with overlapSumSqWeights', () => {
      ...
      expect(sql).toContain(
        'SUM("weight" * "weight") FILTER (WHERE "Q5_1" = 1 AND "Q5_2" = 1)::DOUBLE as overlapSumSqWeights',
      );
    });
  });
  ...
  describe('buildDrillDownQuery', () => {
    ...
    it('includes column filter when provided', () => {
      ...
      expect(sql).toBe(`SELECT * FROM main WHERE "Gender" = 'Male' AND "Region" = 'North' LIMIT 100 OFFSET 0`);
    });
  ...
  describe('buildFilterClause', () => {
    ...
    it('handles eq operator', () => {
      const filters: Filter[] = [{ id: 'f1', variableId: 'Gender', operator: 'eq', value: 1 }];

      expect(buildFilterClause(filters)).toBe('"Gender" = 1');
    });

    it('handles neq operator', () => {
      const filters: Filter[] = [{ id: 'f1', variableId: 'Gender', operator: 'neq', value: 1 }];
    ...
    it('handles lt operator', () => {
      const filters: Filter[] = [{ id: 'f1', variableId: 'Age', operator: 'lt', value: 65 }];

      expect(buildFilterClause(filters)).toBe('"Age" < 65');
    });

    it('handles in operator with array', () => {
      const filters: Filter[] = [{ id: 'f1', variableId: 'Status', operator: 'in', value: [1, 2, 3] }];

      expect(buildFilterClause(filters)).toBe('"Status" IN (1, 2, 3)');
    });

    it('throws error for in operator without array', () => {
      const filters: Filter[] = [{ id: 'f1', variableId: 'Status', operator: 'in', value: 1 }];
