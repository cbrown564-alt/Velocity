...
describe('placeVariableSet', () => {
  ...
  describe('non-grid', () => {
    ...
    it('drop-zone-rows is a no-op when set is already in rows', () => {
      expect(placeVariableSet('age', 'single', 'drop-zone-rows', { rowVars: ['age'], colVar: null })).toBeNull();
    });
    ...
    it('canvas places second variable on columns', () => {
      expect(placeVariableSet('region', 'single', 'canvas', { rowVars: ['gender'], colVar: null })).toEqual({
        colVar: 'region',
      });
    });
  });
  ...
  describe('grid', () => {
    ...
    it('delegates drop-zone-rows to applyGridSetDrop', () => {
      expect(placeVariableSet(setId, 'grid', 'drop-zone-rows', { rowVars: ['other'], colVar: null })).toEqual(
        applyGridSetDrop(setId, 'drop-zone-rows', { rowVars: ['other'], colVar: null }),
      );
    });
...
describe('applyGridSetDrop', () => {
  ...
  it('drop-zone-rows appends scale and sets items column', () => {
    expect(applyGridSetDrop(setId, 'drop-zone-rows', { rowVars: ['other_var'], colVar: null })).toEqual({
  ...
  it('drop-zone-cols sets scale column and appends items to rows', () => {
    expect(applyGridSetDrop(setId, 'drop-zone-cols', { rowVars: ['other_var'], colVar: null })).toEqual({
  ...
  it('canvas delegates to gridSetToTableConfig full mode', () => {
    expect(applyGridSetDrop(setId, 'canvas', { rowVars: ['other_var'], colVar: 'other_col' })).toEqual(
