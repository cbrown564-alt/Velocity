...
export function recommendChart(context: RecommenderContext): ChartRecommendation {
  ...
  const isSyntheticGrid =
    primaryRowVar?.synthetic && primaryRowVar?.sourceGridId && colVar?.synthetic && colVar?.sourceGridId;

  if (isSyntheticGrid || isGrid) {
    // Keep isGrid for backward compatibility during migration
  ...
  if (colVar) {
    ...
    const hasCategorical =
      (primaryRowVar ? isCategoricalType(primaryRowVar.type) || isOrderedType(primaryRowVar.type) : false) ||
      isCategoricalType(colVar.type) ||
      isOrderedType(colVar.type);
