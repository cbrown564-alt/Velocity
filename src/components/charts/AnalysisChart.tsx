...
export const AnalysisChart: React.FC<AnalysisChartProps> = ({
  ...
  const renderContent = () => {
    ...
    switch (effectiveChartType) {
      ...
      case 'box-plot': {
        const boxStats =
          variableStats ||
          (displayChartData?.series[0]?.stats ? { stats: displayChartData.series[0].stats } : undefined);
      ...
      default: {
        ...
        return <div className={styles.placeholder}>Chart type '{unhandledType}' not yet implemented</div>;
      }
    }
  };
  ...
  const legendItems =
    displayChartData?.series.map((s, i) => ({
      label: s.label,
  ...
  return (
    <div className={`${styles.container} ${className}`}>
      ...
      <div className={styles.toolbar}>
        <div className={styles.toolbarLeft}>
          <ChartSelector
            ...
            availableTypes={
              recommendation?.alternatives ? [recommendation.default, ...recommendation.alternatives] : undefined
            }
            onSelect={(type) => {
              if (activeSlideId) {
        ...
        {config.showLegend && legendItems.length > 0 && <ChartLegend items={legendItems} className={styles.legend} />}
      ...
      <div
        ...
        aria-label={
          displayChartData
            ? `Chart showing ${displayChartData.rowVariables.map((v) => v.label).join(', ')}`
            : 'Analysis chart'
        }
      >
      ...
      {displayChartData && <ChartScreenReaderTable data={displayChartData} />}
        ...
        {data.series[0]?.data.map((point, idx) => (
          <tr key={point.rawValue}>
            ...
            {data.series.map((series) => {
              ...
              return (
                <td key={series.key}>{cell ? `${cell.value.toLocaleString()} (${cell.percent.toFixed(1)}%)` : '—'}</td>
              );
            })}
            {hasMultipleCols && (
              <td>
