...
export const DivergingBarRenderer: React.FC<BaseChartRendererProps> = ({
  ...
  const xDivScale = d3.scaleLinear().domain([-maxDiverging, maxDiverging]).range([0, divergingPixelWidth]);
  ...
  const handleRowContextMenu = useCallback(
    (rowLabel: string, event: React.MouseEvent) => {
          ...
          <div className="flex flex-wrap justify-center gap-x-4 gap-y-1 px-4 text-[10px] text-[var(--text-secondary)] h-full overflow-y-auto content-center">
            {legendItems.map((item) => (
              <div key={item.label} className="flex items-center gap-1">
                ...
                <span className="max-w-[180px] whitespace-normal break-words leading-tight" title={item.label}>
                  {item.label}
                </span>
              </div>
            ))}
