...
export const HexbinRenderer: React.FC<BaseChartRendererProps> = ({ width, height, processedData }) => {
  ...
  const { bins, hexagonPath } = useMemo(() => {
    const hexbinGenerator = d3Hexbin()
      ...
      .x((d) => xScale(d[0]))
      .y((d) => yScale(d[1]));
    return { bins: hexbinGenerator(points), hexagonPath: hexbinGenerator.hexagon() };
  ...
  const opacityScale = d3scale.scaleLinear().domain([0, maxCount]).range([0.1, 0.8]); // Holographic range
        ...
        <g transform={`translate(0,${innerHeight})`}>
          ...
          {xScale.ticks(5).map((tick) => (
            <g key={tick} transform={`translate(${xScale(tick)},0)`}>
              ...
              <text y={16} textAnchor="middle" className="text-[10px] fill-[var(--viz-text-axis)] font-mono">
                {tick}
              </text>
            </g>
          ))}
