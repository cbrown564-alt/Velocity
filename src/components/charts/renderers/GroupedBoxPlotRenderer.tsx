...
export const GroupedBoxPlotRenderer: React.FC<BaseChartRendererProps> = ({ width, height, colors, processedData }) => {
  ...
  const rowGroups = processedData.rows
    .map((row, i) => {
  ...
  const colGroups = processedData.columns
    .map((col, i) => {
        ...
        {groups.map((g) => {
          ...
          return (
            <g key={g.label}>
              ...
              <title>
                {g.label}: Min={min.toFixed(1)}, Q1={q1.toFixed(1)}, Median={median.toFixed(1)}, Q3={q3.toFixed(1)},
                Max={max.toFixed(1)}
