...
export const ViolinRenderer: React.FC<BaseChartRendererProps> = ({
  ...
  const yScale = d3scale.scaleLinear().domain([minVal, maxVal]).range([innerHeight, 0]).nice();
        ...
        {groups.map((g, i) => {
          ...
          return (
            <g key={g.label} transform={`translate(${x},0)`}>
              <path d={areaGenerator(g.data) || ''} fill={color} fillOpacity={0.2} stroke={color} strokeWidth={1} />
            </g>
          );
        })}
      </g>
