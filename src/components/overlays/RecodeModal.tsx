    ...
    if (isOpen && variable) {
      const loadValues = async () => {
        ...
        try {
          ...
          const defaultMode =
            allowsNumericStats(variable.type, variable.orderedScoring) || variable.type === 'date'
              ? 'binning'
              : 'categorical';
          setMode(defaultMode);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)] font-body">
              Group values from <span className="font-semibold text-[var(--text-primary)]">{variable?.label}</span> into
              a new variable.
            </p>
          </div>
        </div>
              ...
              {loading ? (
                ...
                uniqueValues.map((val) => {
                  ...
                  return (
                    <div
                      key={val}
                      className="grid grid-cols-[1fr_auto_1fr] gap-4 p-3 items-center hover:bg-[var(--bg-surface)] transition-colors group"
                    >
