...
export const FilterModal: React.FC<FilterModalProps> = ({ isOpen, onClose, variables, onSave }) => {
  ...
  const filteredVariables = useMemo(() => {
    const categoricalVars = variables.filter(
      (v) => isCategoricalType(v.type) || isOrderedType(v.type) || v.valueLabels.length > 0,
    );

    if (!searchQuery.trim()) return categoricalVars;

    const query = searchQuery.toLowerCase();
    return categoricalVars.filter((v) => v.label.toLowerCase().includes(query) || v.name.toLowerCase().includes(query));
  ...
  const handleValueToggle = (value: number | string) => {
    setSelectedValues((prev) =>
      (prev as any[]).includes(value) ? prev.filter((v) => v !== value) : ([...prev, value] as any),
    );
  };
                ...
                filteredVariables.map((variable) => (
                  ...
                  >
                    ...
                    <p className="text-xs truncate font-mono" style={{ color: 'var(--text-secondary)' }}>
                      {variable.name} •{' '}
                      {variable.valueLabels.length > 0 ? `${variable.valueLabels.length} values` : variable.type}
                    </p>
                  </button>
                ))
              )}
          ...
          <>
            ...
            <div className="max-h-64 overflow-y-auto custom-scrollbar space-y-1">
              {loadingValues ? (
                ...
                <p className="text-sm text-center py-8 text-[var(--text-tertiary)]">No values found</p>
              ) : (
                availableValues.map((vl) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs ml-auto shrink-0 font-mono" style={{ color: 'var(--text-secondary)' }}>
                        ({vl.value})
                      </span>
                    </button>
                  );
                ...
                style={{
                  ...
                  boxShadow:
                    selectedValues.length > 0
                      ? '0 0 15px color-mix(in srgb, var(--color-accent), transparent 80%)'
                      : 'none',
                }}
              >
