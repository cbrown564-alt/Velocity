...
import { normalizeStoredSessionState, sessionStateToStorePatch } from './session';
import type {
  DataTransform,
  Dataset,
  Folder,
...
} from '../../store/slices/data/types';
import {
  buildVariableSetsFromVariables,
  normalizeVariable,
  normalizeVariableSet,
} from '../../store/slices/data/variableNormalization';
...
export async function rehydrateDatasetFromOpfsSource(
      ...
      if (status.hasData) {
        if (runAnalysis) {
          void runAnalysis().catch((error) => {
            console.warn('[workspaceBoot] Analysis replay failed during OPFS rehydration shortcut:', error);
          });
        }
        return;
      }
  ...
  console.log(`[workspaceBoot] Rehydrating DuckDB from OPFS source: ${dataset.opfsFileKey}`);
  ...
  if (transformLog.length > 0) {
    console.log(`[workspaceBoot] Replaying ${transformLog.length} transforms`);
  }
  ...
  for (const transform of transformLog) {
    ...
    try {
      ...
      console.warn('[workspaceBoot] Transform replay failed:', message);
    }
  }
