...
export function createDatasetActions(
  ...
  return {
    ...
    loadSAV: async (fileName: string, buffer: ArrayBuffer, options?: { datasetId?: string; opfsFileKey?: string }) => {
      ...
      console.log(
        `📊 [DataSlice] SAV loaded: ${response.rowCount} rows, ${variables.length} variables, ${variableSets.length} variable sets in ${response.durationMs.toFixed(2)}ms`,
      );
      if (datasetId) {
        void browserEngine.updatePersistenceMetadata({
          datasetId,
    ...
    loadSAVMetadata: async (fileName: string, buffer: ArrayBuffer) => {
      ...
      console.log(
        `📊 [DataSlice] SAV metadata loaded: ${response.rowCount} rows, ${variables.length} variables in ${response.durationMs.toFixed(2)}ms`,
      );
    },
    ...
    loadSAVSample: async (
      ...
      set({
        dataset: {
          ...
          loadDiagnostics: {
            ...
            reason: 'sampling',
            message:
              'Loaded using sampled metadata to reduce memory risk. Value labels may be incomplete until full load.',
            createdAt: Date.now(),
          },
        },
        variableSets,
      ...
      console.log(
        `📊 [DataSlice] SAV sample loaded: ${response.sampleRowCount}/${response.rowCount} rows (${response.sampleStrategy}), ${variables.length} variables in ${response.durationMs.toFixed(2)}ms`,
      );
    },
