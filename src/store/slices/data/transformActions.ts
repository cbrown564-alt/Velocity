...
export function createTransformActions(
  set: DataSliceSet,
  get: DataSliceGet,
): Pick<DataSlice, 'recodeVariable' | 'fillSystemMissing' | 'deleteGroupedVariable' | 'splitGroupValue'> {
  return {
    ...
    deleteGroupedVariable: async (varId: string): Promise<void> => {
      ...
      set((state) => {
        ...
        const newVariableSets = state.variableSets.filter((vs) => !vs.variableIds.includes(varId));
        ...
        return {
          ...
          variableStats: Object.fromEntries(Object.entries(state.variableStats).filter(([key]) => key !== varId)),
        };
      });
    ...
    splitGroupValue: async (varId: string, groupValue: string): Promise<void> => {
      ...
      for (const [key, val] of Object.entries(transform.config.mappings)) {
        if (val === groupValue) {
          const originalLabel = sourceVar?.valueLabels.find((vl) => String(vl.value) === key)?.label ?? key;
      ...
      set((state) => ({
        transformLog: state.transformLog.map((t) => (t.newColId === varId ? { ...t, config: newConfig } : t)),
        variableStats: Object.fromEntries(Object.entries(state.variableStats).filter(([key]) => key !== varId)),
