...
export const createUISlice: StateCreator<UISlice, [], [], UISlice> = (set) => ({
  ...
  toggleVariableSetSelection: (id, multi = false) =>
    set((state) => {
  ...
  selectVariableSetRange: (id, allIds) =>
    set((state) => {
  ...
  showWaveDetectionBanner: (bannerState) =>
    set({
      waveDetectionBanner: { ...bannerState, isVisible: true },
    }),

  dismissWaveDetectionBanner: () =>
    set({
      waveDetectionBanner: { ...DEFAULT_WAVE_BANNER },
    }),
  ...
  toggleTableDensity: () =>
    set((state) => ({ tableDensity: state.tableDensity === 'compact' ? 'generous' : 'compact' })),
  ...
  addToast: (toast) =>
    set((state) => {
      ...
      return {
        toasts: next.length > MAX_VISIBLE_TOASTS ? next.slice(-MAX_VISIBLE_TOASTS) : next,
      };
    }),
  dismissToast: (id) =>
    set((state) => ({
