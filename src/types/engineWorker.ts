...
import type { WorkerAnalysisSettings, WorkerAnalysisContext, PersistedMetadata, VariableStatsResult, VariableStatsFrequency, NumericStats } from './worker';
...
export type EngineWorkerRequest = EngineRequestBase & (
  ...
  | { type: 'engine.getVariableStats'; column: string; variableType?: VariableType; orderedScoring?: OrderedScoring; binCount?: number; missingValues?: MissingValueDef }
...
export type EngineWorkerResponse = EngineResponseBase & (
  ...
  // --- Persistence ---
  | { type: 'engine.persistenceStatus'; opfsAvailable: boolean; mode: 'opfs' | 'memory' | 'disabled'; dbPath: string; lastError?: string }
  | { type: 'engine.corruptionDetected'; message: string }
  | { type: 'engine.persistedDataFound'; schema: { name: string; type: string }[]; rowCount: number; metadata?: PersistedMetadata }
  ...
  | { type: 'engine.savLoaded'; variables: Variable[]; variableSets: VariableSet[]; rowCount: number; durationMs: number }
  | { type: 'engine.savMetadataLoaded'; variables: Variable[]; variableSets: VariableSet[]; rowCount: number; durationMs: number }
  | { type: 'engine.savSampleLoaded'; variables: Variable[]; variableSets: VariableSet[]; rowCount: number; sampleRowCount: number; sampleStrategy: 'sequential' | 'spread'; durationMs: number }
  | { type: 'engine.loadProgress'; phase: 'parsing' | 'inserting' | 'complete'; progress: number; rowsProcessed?: number; totalRows?: number; message: string }

  // --- Querying ---
  | { type: 'engine.queryResult'; data: Array<AggregatedRow | Record<string, unknown>>; durationMs: number; tableStats?: TableStats }
...
export type EngineResponseByType<T extends EngineWorkerResponse['type']> =
  Extract<EngineWorkerResponse, { type: T }>;
