...
const gridExpansion = ['id,q1_1,q1_2,q1_3', '1,1,2,3', '2,2,3,1', '3,3,1,2', '4,1,1,1', '5,5,5,5'].join('\n');
writeFileSync(join(FIXTURES_DIR, 'grid_expansion.csv'), gridExpansion);

// 4. filtered_query.csv
const filteredQuery = ['id,city,salary', '1,NY,5000', '2,NY,6000', '3,SF,7000', '4,SF,8000', '5,CHI,4000'].join('\n');
writeFileSync(join(FIXTURES_DIR, 'filtered_query.csv'), filteredQuery);

// 5. empty_cells.csv
const emptyCells = ['id,varA,varB', '1,A1,B1', '2,A1,B1', '3,A2,B2', '4,A2,B2'].join('\n');
writeFileSync(join(FIXTURES_DIR, 'empty_cells.csv'), emptyCells);

// 6. all_missing_column.csv
const allMissing = ['id,valid_var,missing_var', '1,A,', '2,B,', '3,A,'].join('\n');
