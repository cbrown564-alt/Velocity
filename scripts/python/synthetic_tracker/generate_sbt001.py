...
sample=ds[0]; cb={'generatorVersion':VERSION,'projectId':'SBT-001','missingCodes':{'97':'structural/not applicable','98':"don't know",'99':'refused/prefer not to say'},'variables':[]}
...
processing={'projectId':'SBT-001','version':'2.0','steps':[{'id':'weight','action':'set_weight','variable':'wt_final'},{'id':'image_base','action':'define_base','rule':'Brand-image items use respondents with F1_[brand] in {3,4,5}; raw 97 is structural and excluded.'},{'id':'service_base','action':'define_base','rule':'E5 service satisfaction uses tracked-brand customers with E4_contacted_service == 1.'},{'id':'missing','action':'user_missing','rule':'Apply variable-specific codebook missing values; never globally collapse 97/98/99.'},{'id':'nps','action':'derive','rule':'Promoter E2_nps>=9; detractor<=6; NPS=100*(promoter%-detractor%) among current-provider respondents.'}]}
...
validation={'canonicalSha256':{str(w):hashlib.sha256((OUT/'raw'/f'wave_{w:02d}.csv').read_bytes()).hexdigest() for w in range(1,6)},'environment':{'python':platform.python_version(),'numpy':np.__version__,'pandas':pd.__version__,'scipy':scipy.__version__},'projectId':'SBT-001','generatorVersion':VERSION,'rootSeed':SEED,'selectedSeed':seed,'rejectedSeeds':rejected,'hardGates':hard,'metrics':{k:float(v) for k,v in mm.items()},'structural':struct,'lowBaseTrap':mosaic,'status':'PASS' if all(hard.values()) else 'FAIL','savExport':{'status':'OPTIONAL','note':'CSV is canonical; SAV conversion requires a compatible writer such as pyreadstat.'}}
