...
| INF-06 | Consideration Top-2-Box nets are derived one `recode` per brand (5 calls for a 5-brand grid); no reusable "net" primitive applies a `{4,5}→T2B` grouping across a brand/attribute grid in one step | `scripts/brand-tracker-recipe-demo.ts` (Phase B recipe, steps 6/8), `docs/workstreams/deck_native/10_brand_tracker_demo_plan.md` §4.2 | `NET-DERIVE` | Medium |
| INF-07 | NPS *score* (%promoters − %detractors as a single displayed stat) cannot be produced with current `recode`/`crosstab`; the recipe demos promoter/passive/detractor shares as a demo-around | `scripts/brand-tracker-recipe-demo.ts` (Phase B recipe, step 7), plan §4.2 disposition table | `NET-DERIVE` | Medium |
| INF-08 | Cross-wave harmonization produces a NEW stacked output table; there is no in-place rename on the active dataset, and harmonization mappings are not captured in `transformLog`/session, so the mapping cannot be replayed on reopen or reused on the next wave | `scripts/brand-tracker-recipe-demo.ts` (Phase B recipe, step 3), `src/engine/workspaceManager.ts`, `src/core/session/sessionExporter.ts` | `META-HYGIENE` (secondary `RECIPE-REPLAY`) | Medium |
| INF-09 | Multi-break banner tables are composed as one `crosstab` per break (segment, age_band, region …); there is no persistent banner plan reused across a deck's exhibits | `scripts/brand-tracker-recipe-demo.ts` (Phase B recipe, step 8) | `BANNER-PLAN` | Medium |

> **Phase B note (PILOT-DEMO-2).** INF-06…INF-09 were surfaced by driving the brand-tracker transformation recipe (plan §4.1) against the real engine with `scripts/brand-tracker-recipe-demo.ts`. They confirm and make concrete the earlier inferred signals — INF-06/07 sharpen INF-02 (`NET-DERIVE`), INF-08 relates to INF-04 (`RECIPE-REPLAY`), and INF-09 sharpens INF-03 (`BANNER-PLAN`). They remain **internal** signals, not external pilot evidence. A short walkthrough of what worked, what was demoed-around, and each gap lives in `docs/workstreams/deck_native/brand_tracker_recipe_gap_log.md`.
