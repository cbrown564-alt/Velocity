...
npm run test:e2e -- tests/e2e/crosstab-virtualization.spec.ts tests/e2e/visual-polish-crosstab.spec.ts
```
...
- Drag-to-merge and sticky headers remain usable. (Preserved by design — the `<table>`/`<thead sticky>` structure and `data-merge-*` attributes are unchanged; only the `<tbody>` is windowed. The e2e asserts the sticky header survives scrolling; `visual-polish-crosstab.spec.ts` confirms the common path is unregressed. The one bounded limitation is label resolution for off-screen multi-select merge *sources*, documented above.)
