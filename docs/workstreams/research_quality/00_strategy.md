...
The [capped second-pass comparison](../../../evals/research_quality/runs/2026-09-completeness-comparison/readout.md) is closed. Its scored SBT-004 pair shows that a general review and the structured checklist both recovered the missing mandatory contrast. SBT-005/006 drafts already had full mandatory recall, and one SBT-006 checklist call failed under the no-retry protocol. Retrospective scoring on 26 September closes the three previously unscored outputs: mandatory findings are preserved, mapped numerical checks pass, and the SBT-006 pair remains incomplete. This evidence does not justify promoting the checklist. The separate generation guide describes subsequent corpus planning; they do not change the pilot conclusion or imply that generation is complete.
