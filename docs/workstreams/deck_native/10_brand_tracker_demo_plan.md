...
- **Phase A — Dataset (foundation).** Generator + config + ground truth; commit waves; golden test green; `.gitignore` allow-list; `scripts/python/README.md` updated. *Gate: engine reproduces ground truth on all committed waves (T/L/U/G).*
- **Phase B — Recipe.** Execute §4.1 end to end via MCP against the raw wave; capture the gap log (§4.2) into `pilot_04a` §7 internal signals; session export/reopen replays transforms. *Gate: scripted run completes with zero manual repair; session round-trip test.*
- **Phase C — Deck.** `scripts/brand-tracker-demo.ts`; deck built, exported, inspected; scored against §8 checklist + `09` rubric; exemplar candidate artifacts committed. *Gate: rubric deck-level pass; inspect-pptx clean; PR evidence per `07_..._v2.md` §13 (including exemplar diff).*
- **Phase D — Integration + refresh.** Load Example swap, autoFirstCrosstab, demo contracts, e2e, wave-5 refresh demo, EVAL-07 freeze, tracker/docs sync. *Gate: e2e green; EVAL-07 artifacts frozen; consultant sign-off requested for exemplar promotion.*

### Proposed tracker rows (add to `tracker_00_implementation_status.md` §4.1 on adoption, per §6 update rules)
