...
=======
**Current finding (July 29):** `DESIGN-CONV-R0` reconciliation is complete — see [`docs/cursor-capacity-sprint/R0_RECONCILIATION.md`](cursor-capacity-sprint/R0_RECONCILIATION.md). `DESIGN-CONV-K2` slide weight/settings restore is In review on this line. Paths B–I and Q5 remain **Candidate** until their Wave 1 PRs merge. F is rejected for this sprint; E is deferred until after the core journey. Export preview lane (B), unified palette grammar (K1), and Wave 2 chrome/discovery candidates remain open. Hidden-but-focusable inspector controls, contrast, slide-title typography, and fixed-width risks remain under K3.
>>>>>>> 87fa887 (feat(store): restore per-slide weight and settings (DESIGN-CONV-K2))
