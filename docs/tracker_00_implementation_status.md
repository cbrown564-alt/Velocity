...
2. Wave 1 (`K1`, `K2`, `K3`, `B`, `Q5`) is complete. Verify retained Wave 2 candidates (`D`, `C`, `H`, `G`, `I`).
...
**Current finding (September 21):** Wave 1 core items `R0` / `Q5` / `K1` / `K2` / `K3` are Done on `main`. `DESIGN-CONV-B` export preview lane is Done on `main`. Wave 2 is integrated locally on `codex/convergence-candidates` with local automated verification (September 21); this is not a main-branch promotion or representative-user validation. C remains **Candidate** pending discovery evidence; D/H/G/I are **In review**. F rejected; E deferred. Local checks: lint, all typechecks, 1,632 unit tests (3 todo), build, 28 product/boot browser tests, 4 production smoke tests, formatting, architecture guards, and journey gate passed. The browser tests cover upload/palette handoff, accessible pinning and rail expansion, session import diagnostics, reload, and dataset switching. The journey measured 2.35s cold / 1.49s warm first crosstab and 0.14s PPTX generation. Representative-user discovery evidence and main-branch promotion remain outstanding.
...
| DESIGN-CONV-A | A | Validate | Fresh final photography; reproducible journey automation; 3–5 unscripted sessions scored for time, discovery, errors, and recovery | DESIGN-CONV-B, C, D, G, H, I, Q5, K1, K2, K3 | Blocked | No | I,V | Evidence kit from PR #59 merged: [`design_conv_a_unscripted_session_scorecard.md`](design_conv_a_unscripted_session_scorecard.md), five blank session cards, and screenshot command. July images remain historical. Still required: fresh final-candidate photography and 3–5 observed unscripted sessions; no user-validation claim |
...
| DESIGN-CONV-C | C | Discovery | Thin collapsible recent-and-pinned variable strip, not a full sidebar | DESIGN-CONV-R0 | Candidate | Yes | T,L,U,I,V | Integrated from #54 on `codex/convergence-candidates`: removed retired focus-mode references; unavailable pins cannot consume visible slots; accessible pin controls. Remains Candidate until DESIGN-CONV-A establishes discovery value. |
| DESIGN-CONV-D | D | Keyboard | First-palette inline ghost that teaches the canonical row/column grammar and dismisses permanently | DESIGN-CONV-R0, DESIGN-CONV-K1 | In review | Yes | T,L,U,I | Integrated from #52 on `codex/convergence-candidates`: Grammar A onboarding, per-dataset dismissal, storage-failure fallback; storage and browser dismissal regressions pass; awaiting review and promotion. |
...
| DESIGN-CONV-G | G | Polish | Story rail collapses below 48px for small decks and expands predictably | DESIGN-CONV-R0 | In review | Yes | T,L,U,I | Integrated from #49 on `codex/convergence-candidates`: explicit keyboard/touch toggle, 44px single-slide rail, preserved K2 summaries and K3 contrast; warnings keep rail expanded. Browser keyboard expansion and pinning pass; global Enter no longer intercepts native buttons. |
| DESIGN-CONV-H | H | Continuity | After upload, land on slide 1 with the insertion path ready | DESIGN-CONV-R0, DESIGN-CONV-K1 | In review | Yes | T,L,U,I,V | Integrated from #58 on `codex/convergence-candidates`: first-upload handoff with live-state/example-auto-analysis guard; preserved recipe assertions and export review. Browser upload, escape, reload, and workspace-switch journeys pass. |
| DESIGN-CONV-I | I | Agent | Quiet imported-session summary: slides added, unresolved variables, recipe changes | DESIGN-CONV-R0, DESIGN-CONV-K2 | In review | Yes | T,L,U,I,A | Integrated from #56 on `codex/convergence-candidates`: persistent import summary, current app/recovery wiring and active-slide restoration retained; single-slide warnings remain visible. Browser export/import round trip verifies unresolved references, affected slides, analysis, and dismissal. |
