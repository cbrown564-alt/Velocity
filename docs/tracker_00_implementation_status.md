...
| PILOT-3 | PowerPoint Loop | Complete the high-value PPTX loop: client template import/map, editable object preservation, saved slide recipes, dataset/wave replacement, review-before-export | PILOT-1, PILOT-2 | In progress | Yes | T,L,U,I,A,V | `src/core/export/slideRecipe.ts` — slide recipe contract, dataset-replacement assessment, review-before-export guard (`slideRecipe.test.ts`, `ExportModal.test.tsx`); `src/core/export/templateMapping.ts` — initial headless template placeholder/mapping scaffold + applicability checks (`templateMapping.test.ts`); wave refresh and editable-object preservation still open |
