...
| DESIGN-CONV-C | C | Discovery | Thin collapsible "recent & pinned" variable strip below toolbar (~32px); not a full sidebar wall | DESIGN-CONV-A | In review | Yes | T,L,U,I,V | Accent budget audit in PR (strip uses neutral tokens + shelf markers only; no accent chrome). Drag-first chips via DnD. Q1 final keep/cut still paired with D pending A evidence. |
