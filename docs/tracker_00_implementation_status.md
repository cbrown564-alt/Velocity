...
| DESIGN-CONV-B | B | Journey | Export preview lane: thumbnail filmstrip of export-bound slides, recipe summary, significance audit — review step before PPTX download | DESIGN-RESET-1 | In review | Yes | T,L,U,I,V | Branch `cursor/design-conv-b-export-preview`; unit tests (exportPreviewSummary, ExportPreviewLane, ExportModal preview gate); e2e pilot + brand-tracker require preview before PPTX; PR pending |
