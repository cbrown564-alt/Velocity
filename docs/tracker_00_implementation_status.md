...
| DESIGN-CONV-E | E | Story | Workspace "Start from template" → 3-slide tracker skeleton (awareness · consideration · preference); story rail pre-populated | DESIGN-CONV-B, PILOT-DEMO-3 | In review | Yes | T,L,U,I,V | `src/core/deckTemplates/*` + `applyDeckTemplate`; empty-state CTA; unit tests; e2e `tests/e2e/brand-tracker-template.spec.ts`. Prefer merge after DESIGN-CONV-B |
