...
| PILOT-3 | PowerPoint Loop | Complete the high-value PPTX loop: client template import/map, editable object preservation, saved slide recipes, dataset/wave replacement, review-before-export | PILOT-1, PILOT-2 | In review | Yes | T,L,U,I,A,V | Delivered minimal loop mechanics with focused coverage: `src/core/export/slideRecipe.ts` now supports selected-scope dataset/wave replacement review with active-slide analysis overrides in `buildExportReview` (tested in `src/core/export/slideRecipe.test.ts`); `src/core/export/templateMapping.ts` provides template applicability gating via `buildTemplateApplicabilityReview` + mapping checks (tested in `src/core/export/templateMapping.test.ts`); `src/components/overlays/ExportModal.tsx` unifies review-before-export warnings/blocks across replacement + template applicability and enforces blocking gates before export (tested in `src/components/overlays/ExportModal.test.tsx`). Gate still open for `Done`: wire full user-facing template import/mapping persistence and binary template application path (`baseTemplate` + `applyTemplateBindings`) in production flow, then validate with integration/E2E evidence. |
