...
| STAB-UI-B | Canvas Polish | Smart empty states with suggested variables; adaptive shelf collapse (empty shelves minimize); crosstab typographic hierarchy; Focus Mode (`F` shortcut); toast/feedback layer for operations | STAB-UI-A | Done | No | T,U,I,A | `src/store/slices/uiSlice.ts` (focusMode, toasts), `src/components/common/ToastLayer.tsx` + `.test.tsx`, `src/components/layout/AppShell.tsx` + `.test.tsx` (F shortcut, focus mode exit on Manager open), `src/features/dashboard/DashboardShell.tsx` (adaptive shelves, focus mode chrome hiding, toast wiring), `src/features/dashboard/components/SlideContainer.tsx` (smart empty state with suggested variables), `src/features/dashboard/hooks/useSuggestedVariables.ts` + `.test.ts`, `src/features/dashboard/components/DataTable.tsx` (typographic hierarchy: mono cells, semibold labels, uppercase headers)
| STAB-UI-C | Theme & Density | Shadow token fix (remove hardcoded `rgba(0,0,0,…)`); theme preview cards in switcher; variable list visual weight (derived badges, shelf color indicators); command palette MVP (`Cmd+K`); keyboard shortcut reference (`?`) | STAB-UI-B | Done | No | T,U,A | `src/index.css` (shadow tokens), `tailwind.config.cjs`, `scripts/check-design-tokens.mjs`, `src/components/common/ThemeSwitcher.tsx`, `src/features/dashboard/components/DraggableVariable.tsx` + `.test.tsx`, `src/components/common/CommandPalette.tsx` + `.test.tsx`, `src/components/common/KeyboardShortcuts.tsx` + `.test.tsx`, `src/components/layout/AppShell.tsx`, `src/store/slices/uiSlice.ts` |
| STAB-UI-D | UI/UX review remediation | Close P1 findings from comprehensive review (UXR-037, 040, 008, 010, 024); modal Esc/focus (041–042); OPFS user-facing copy (047); responsive Canvas chrome (044–045) | STAB-UI-C | Not started | No | T,U,A | Evidence: `docs/reviews/ui_ux_review_2026-05/findings.md`, `session-12-synthesis.md`, `docs/audit_06_ui_ux_review_2026-05.md` |
| STAB-UI-E | Back-Room Delight | Variable Manager diagnostic layer (Vitals, Smart Facets, Living Inspector, Recode Chronicle); Workspace library layer (Portraits, Timeline, Ambient Search); Onboarding layer (Guided First Finding, Confidence Builder, Returning Researcher) | STAB-UI-C | In progress | No | T,U,A | **Quick wins 9.1–9.4 Done.** **Phase 1 (Diagnostic) Done:** Smart Facets — `FacetedSearchBar.tsx` + `.module.css`. **Phase 2 (Connection) Done:** Living Inspector + Recode Chronicle (minimal). **Phase 3 (Library) Done:** Dataset Portraits — `DatasetPortrait.tsx` + `workspaceLibrary.ts` (color signature, activity heatmap, slide thumbnail, session badge); Project Timeline — `WaveTimeline.tsx` (delta preview on hover, wave gap placeholders, harmonization ring on `ProjectCard`); Ambient Search — `WorkspaceView.tsx` (variable-keyword search, semantic hints, dynamic category chips). **Pending:** Phase 4 (Onboarding: Confidence Builder, Returning Researcher). |
