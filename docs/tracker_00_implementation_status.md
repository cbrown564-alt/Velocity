...
| STAB-UI-E | Back-Room Delight | Variable Manager diagnostic layer (Vitals, Smart Facets, Living Inspector, Recode Chronicle); Workspace library layer (Portraits, Timeline, Ambient Search); Onboarding layer (Guided First Finding, Confidence Builder, Returning Researcher) | STAB-UI-C | Done | No | T,U,A | **Quick wins 9.1–9.4 Done.** **Phases 1–4 Done (2026-05-21):** Smart Facets, Living Inspector, Recode Chronicle, Dataset Portraits, Project Timeline, Ambient Search; Phase 4 — `uploadFeedback.ts`, `returningResearcher.ts`, `WelcomeBackCard.tsx`, upload overlay stage labels, format toasts, welcome-back card, deck summary tooltips, Manager change dot. Tests: `workspaceLibrary.test.ts`, `uploadFeedback.test.ts`, `returningResearcher.test.ts`. |
