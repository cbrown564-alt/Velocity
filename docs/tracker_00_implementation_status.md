...
| STAB-UI-T2 | Modal foundation | `ModalShell` dialog semantics, focus trap/restore, form labels, close labels, keyboard click targets (UXT-003–008, UXT-014–016) | STAB-UI-T4 | In progress | Yes | T,L,U,I | **T2.1 Done:** `ModalShell` — `role="dialog"`, `aria-modal`, `useFocusTrap`, default `escapeToClose`; tests in `ModalShell.test.tsx` (UXT-003 fixed). **T2.2–T2.6 in flight:** bypass modal migration (UXT-004), form labels (UXT-006), close labels (UXT-007), keyboard click targets (UXT-008), skip link + VM backdrop (UXT-014), context menus + table scope (UXT-015–016) remain open. |
...
- **T2** T2.1 (`ModalShell` foundation) is Done; T2.2–T2.6 follow-ups remain **In progress** (see plan §T2 and UXT-004/006–008/014–016).
