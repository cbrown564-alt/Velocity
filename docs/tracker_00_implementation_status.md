...
| 2 | **Existing researcher journey** | The design-reset foundation, saved-slide settings, palette grammar, export review, and retained discovery, onboarding, rail, upload-handoff and import-summary changes are on `main` (`913af5e` merged the convergence candidates). On 23 September the brand tracker example reopened with the same weighted crosstab, the PPTX review showed recipe and significance settings, and PPTX/XLSX downloads opened; all 25 browser journeys passed. Direct PowerPoint inspection found a long question used as a mostly empty cover slide. The export format now names the added cover; Excel now labels its numeric total as a count. | Improve the default cover title in the representative workflow, capture current journey screenshots, and use the integrated journey directly to find the next interaction gap. |
