...
2. `DESIGN-CONV-A`: evidence kit ready — refresh screenshots if UI drifted; score 3–5 unscripted first sessions ([`design_conv_a_unscripted_session_scorecard.md`](design_conv_a_unscripted_session_scorecard.md)); do not mark Done until rollup lands.
3. `PILOT-4a`: continue processing gap discovery with external project/file reviews.
4. `PILOT-6`: recruit paid pilots — use post-reset evidence pack for outreach photography ([`docs/assets/design-reset-evidence/`](assets/design-reset-evidence/)).
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| DESIGN-CONV-A | A | Validate | Post-reset PILOT-6 photography; 3–5 unscripted first sessions scored (time-to-first-crosstab, palette discovery rate, interruption count) | DESIGN-RESET-1 | In progress | No | V,I | Evidence kit shipped: [`design_conv_a_unscripted_session_scorecard.md`](design_conv_a_unscripted_session_scorecard.md), [`design-reset-evidence/`](assets/design-reset-evidence/) README + `screenshots/manifest.json`, pending metrics in `before_after_analysis.html`. **Remaining human:** refresh PNGs if UI drifted (`npm run screenshot:design-reset-evidence`; last regen failed — see manifest `regenAttempt`); run ≥3 unscripted sessions; fill rollup; update HTML metrics; then mark Done. Do not invent session scores. |
