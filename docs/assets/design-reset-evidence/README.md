...
**DESIGN-CONV-A status:** The scorecard kit is ready; photography is historical and must be refreshed against the final candidate. Unscripted session metrics in [`before_after_analysis.html`](before_after_analysis.html) stay pending until humans score ≥3 sessions. Do not invent scores.
...
| This pack | `screenshots/` | Historical July 4 UI |
...
Capture to a temporary directory first (`SCREENSHOT_OUT=/tmp/velocity-evidence node scripts/ui-workflow-screenshot-audit.mjs`) and replace the pack only after a complete successful run. If regeneration fails locally, leave existing PNGs in place, note the failure in the PR, and keep `manifest.json` dates truthful (do not bump `capturedAt` without new captures).

## PILOT-6 photography refresh (`DESIGN-CONV-A`)

Do not use the July 4 images as current product photography. Refresh the pack against current `main` before outreach or deck collateral; the paid-pilot readiness requirements remain in [`pilot_06_paid_pilot_program.md`](../../pilot_06_paid_pilot_program.md).
