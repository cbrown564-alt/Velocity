...
*(Wave 3 store migration complete — see TN-W3.5 Done; next structural work in Phase 3 UI cards)*
...
| Card | Evidence |
| :--- | :--- |
| **TN-W3.5** — Remaining direct EngineProxy delegations in store | **Migrated:** `workspaceDatasetLifecycle.ts` transform replay → `browserEngine.recode(sourceColId, { ...config, targetVariableName: newColId })` (mirrors TN-W3.2 `transformActions` pattern). **Grep:** zero `browserEngine.recodeVariable` in `src/store/`. **Intentional transport delegates (no engine-aligned envelope yet):** `harmonizationSlice.buildHarmonizedTable`; `transformActions.fillSystemMissing` / `dropColumn` / `updateColumn`; `persistenceActions` ping / `checkPersistedData` / `flushPersistedData` / `clearPersistedData`; `datasetActions.loadSAVMetadata` / `loadSAVSample` (no `loadBuffer` equivalent — TN-W3.3); `workspaceDatasetLifecycle.ping`. **Tests:** `workspaceDatasetLifecycle.test.ts` — 4 passed (+1 recode replay); `npm run test:run` + `npm run typecheck` on changed files (2026-06-24). **Behavior:** replay uses stored `newColId` as `targetVariableName` (same as prior `recodeVariable` second arg). |
| **TN-W3.0** — Confirm `drillDownSlice` on `browserEngine.query` | **Grep:** `drillDownSlice.ts` — 3 call sites (`openDrillDown` data+count parallel, `loadMoreDrillDown`); zero `engineProxy` / direct `EngineProxy` in slice. **Tests:** drill-down exercised via dashboard integration; full suite — 960 passed (2026-06-24). **Behavior:** unchanged since TN-3.1 facade wiring. |
| **TN-W3.2** — `transformActions` → `browserEngine.recode()` | **Files:** `src/store/slices/data/transformActions.ts` — `recodeVariable` calls `browserEngine.recode(sourceColId, { ...config, targetVariableName: newColName })`; maps `envelope.data.column` → store variable id / transform log / return value (was `response.newColName`). **Grep:** zero `browserEngine.recodeVariable` in `transformActions.ts` (remains in `workspaceDatasetLifecycle.ts` for TN-W3.5). **Tests:** `variableSet.test.ts` — 2 passed; `workspaceDatasetLifecycle.test.ts` — 2 passed; `BrowserEngine.test.ts` — 7 passed; `npm run typecheck` — no errors in migrated files (pre-existing unrelated errors elsewhere); (2026-06-24). **Behavior:** transform log, persistence metadata, `flushPersistedData` unchanged. |
| **TN-W3.1** — `variableCatalogActions` → `runAnalysis('variableStats')` | **Files:** `src/store/slices/data/variableCatalogActions.ts` — `getVariableStats` calls `browserEngine.runAnalysis('variableStats', { column, variableType, orderedScoring, missingValues })`; reads `response.data as VariableStatsResult`. **Facade:** `BrowserEngine.runAnalysis('variableStats')` now forwards `orderedScoring` from config (was hardcoded `undefined`). **Grep:** zero `browserEngine.getVariableStats` in `variableCatalogActions.ts`. **Tests:** `variableCatalogActions.test.ts` — 3 passed (runAnalysis args, cache hit, in-flight guard); `BrowserEngine.test.ts` — 9 passed (+1 orderedScoring/missingValues passthrough); `npm run typecheck` — no errors in migrated files (pre-existing unrelated errors elsewhere) (2026-06-24). **Behavior:** cache, loading flags, orderedScoring/missingValues passthrough preserved. |
...
| **TN-3.6** — Engine ResultEnvelopes for mutations | **Files:** `sessionState.ts` — `setWeight`, `addFilter`, `removeFilter`, `clearFilters`, `commitDeck` now return `ResultEnvelope` via `wrapSync`; `VelocityEngine.ts` delegates unchanged public surface; `types.ts` — mutation result types (`WeightMutationResult`, `FilterMutationResult`, `RemoveFilterResult`, `ClearFiltersResult`, `CommitDeckResult`). **MCP:** `handlers/analysis.ts` (`velocity_filter`, `velocity_clear_filters`, `velocity_set_weight`) and `handlers/deck.ts` (`velocity_commit_deck`) passthrough engine envelopes via `successResponse(result)` (removed ad-hoc `{ ok: true }` payloads). **Tests:** `VelocityEngine.test.ts` — 10 passed (mutation envelope contract); `session-roundtrip.test.ts` — 2 passed; `mcp-server/__tests__/tools.test.ts` — 40 passed (`expectEnvelopeShape` on mutation tools); `tests/e2e/agentWorkflow.test.ts` — 8 passed (Step 8 commit-deck envelope + session round-trip, TN-W3.4); `npm run typecheck:mcp` — pre-existing failures in `crosstabRunner.ts` / `DeckBuilder.ts` (TN-3.9 in flight), no new errors from this card (2026-06-24). |
| **TN-W3.4** — Fix `agentWorkflow` commit-deck envelope test | **Files:** `tests/e2e/agentWorkflow.test.ts` — Step 8 asserts `commitDeck` ResultEnvelope (`operation`, `inputs`, `durationMs`, `warnings`, `metadata`, `data.committedSlides`/`committedSections`) instead of legacy `{ ok: true }`; session export round-trip assertions unchanged. **Tests:** `npm run test:run -- tests/e2e/agentWorkflow.test.ts` — 8 passed; full suite — 956 passed, 1 failed (`variableCatalogActions.test.ts`, TN-W3.1 in flight, unrelated) (2026-06-24). |
...
| **TN-W3.3** — `datasetActions` → `loadBuffer` | **Approach:** Option B — `BrowserLoadBufferResult` (`envelope` + `loaded` raw worker payload); shared `ResultEnvelope<DatasetSummary>` unchanged for MCP/Node. **Migrated:** `datasetActions.loadCSV` / `loadSAV` → `browserEngine.loadBuffer`; `workspaceDatasetLifecycle.rehydrateDatasetFromOpfsSource` → `loadBuffer`. **Not migrated (no loadBuffer equivalent):** `loadSAVMetadata`, `loadSAVSample` — still direct transport; store needs full variables/sets/diagnostics from worker. **Removed duplicate:** `setDatasetContext` in store load paths (now inside `loadBuffer`). **Tests:** `BrowserEngine.test.ts` — 9 passed (+2 loadBuffer); `workspaceDatasetLifecycle.test.ts` — 3 passed (+1 rehydrate); `dataSlice.workspace.test.ts` — 2 passed; `savIngestion.test.ts` — 1 passed; `npm run test:run` — 960 passed, 7 skipped (2026-06-24). **Blocker note:** full schema in envelope deferred — browser store reads `loaded` seam until worker-hosted engine state lands. |
...
**Wave 3 store migration complete (TN-W3.0–W3.5).** Next structural work: Phase 3 UI/convergence cards (e.g. **TN-3.4** Dashboard shell split, **TN-3.5** Workspace CSS) or deferred transport → envelope wrappers when a shared engine method exists.

**Wave 3 store migration order (complete):**
1. ~~`drillDownSlice` — `browserEngine.query()`~~ (TN-W3.0)
2. ~~`variableCatalogActions` — `runAnalysis('variableStats')`~~ (TN-W3.1)
3. ~~`transformActions` — `browserEngine.recode()`~~ (TN-W3.2)
4. ~~`datasetActions` load paths — `loadBuffer`~~ (TN-W3.3)
5. ~~Remaining transport delegations~~ (TN-W3.5 — workspace recode replay migrated; others documented as intentional transport)
...
| Engine + Worker | Partial | BrowserEngine facade done (TN-3.1); store migration complete (TN-W3.0–W3.5) |
| Store | Partial | `dataSlice` split done (TN-2.3); Wave 3 engine facade migration complete; remaining slice `as any` in analysis/drillDown |
